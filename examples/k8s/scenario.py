...
def main() -> None:
    ...
    show("Case 1: GitOps rolling update (config change, limited blast)", evaluator.evaluate(
        DecisionProposal(
            ...
            scope=DecisionScope(asset_ids=["k8s:prod/Deployment/api-server"], max_affected_count=3),
            evidence=[
                EvidenceItem(
                    source="ci-checks",
    ...
    show("Case 2: Scanner deletes running pod (significant, irreversible)", evaluator.evaluate(
        DecisionProposal(
    ...
    show("Case 3: SRE automated rollback (critical, reversible, D-SAL 3)", evaluator.evaluate(
        DecisionProposal(
            decision_type=DecisionType.CONFIGURATION_CHANGE,
