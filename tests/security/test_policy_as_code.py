...
def test_capability_matrix_is_in_policy_not_boundary():
    ...
    assert (
        "self._capability_matrix = capability_matrix" in cap_src
        or "self._capability_matrix" in cap_src
    )
...
def test_capability_injected_into_boundary():
    ...
    policy = DecisionPolicyProvider(
        agent_registry={"a/v1": AgentIdentity("a/v1", 2, frozenset(["data_access"]))},
...
def test_custom_environment_keywords():
    ...
    custom_calc = DSALCalculator(
        environment_rules={"high_risk_keywords": ["prod", "customer-data", "regulated", "pci"]}
    )
...
def test_environment_rules_from_policy():
    ...
    policy = DecisionPolicyProvider(
        environment_rules={"high_risk_keywords": ["prod", "main-cluster", "customer"]}
    )
...
def test_custom_authority_roles():
    ...
    agents = {"a/v1": AgentIdentity("a/v1", 3, frozenset(["remediation", "network_action"]))}
    evaluator = ShaniEvaluator(
...
if __name__ == "__main__":
    ...
    print(
        "  Verifies that the three hardcoded values have been removed and policy.yaml controls them"
    )
    print("=" * 62)
